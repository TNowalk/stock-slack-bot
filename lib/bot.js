...
class Bot {
  ...
  handleMessage(message) {
    ...
    if (!this.getCommand(message) && this.containsSymbol(message.text)) {
          ...
          snapshots.forEach((snapshot) => {
            ...
            if (snapshot.name !== null) {
              this.bot.reply(message, {
                attachments: [{
                  color: this.getSymbolColor(snapshot.change),
                  text: `${this.getSymbolTrend(snapshot.change)}` +
                    ` *${snapshot.name} (${this.getSymbolLink(snapshot.symbol)})*\n` +
                    `$${snapshot.lastTradePriceOnly.format(2)} ${snapshot.change}` +
                    ` (${snapshot.changeInPercent * 100}%)` +
                    ` - ${moment(snapshot.lastTradeDate).format('MMM Do')},` +
                    ` ${snapshot.lastTradeTime}\n` +
                    `Days Range: $${snapshot.daysLow.format(2)}` +
                    ` - $${snapshot.daysHigh.format(2)}\n` +
                    `Volume: ${snapshot.volume.format()}`,
  ...
  getCommand(message) {
    ...
    const matches = message.text.match(/(quote|q|historical|h)/i);
