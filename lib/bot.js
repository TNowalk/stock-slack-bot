...
class Bot {
  ...
  populateLookup(payload) {
    ['users', 'channels', 'groups', 'mpims', 'ims'].forEach((type) => {
    ...
    for (const commandFile of commands) {
      try {
        if (commandFile !== 'command.js') {
          const path = `${__dirname}/commands/${commandFile}`;
          const command = new (require(path))(this.lookup, this.bot, this.storage);
  ...
  handleMessage(message) {
    ...
    if (/^help$/i.test(message.text)) {
      ...
      if (!runCommand) {
        ...
        this.bot.reply(
          message,
          `Could not find a matching command for _${message.text.replace('help', '').trim()}_`
        );
      }
